...
export default class App extends React.Component {
  ...
  componentDidMount() {
    ...
    this.subscribed_events.push(
      Nearby.onDiscoveryStartFailed(({serviceId, statusCode}) => {
        ToastAndroid.showWithGravity(
          'onDiscoveryStartFailed(' + serviceId + ',' + statusCode + ')',
          ToastAndroid.SHORT,
          ToastAndroid.CENTER,
        );
    ...
    this.subscribed_events.push(
      Nearby.onAdvertisingStartFailed(
        ({endpointName, serviceId, statusCode}) => {
          ToastAndroid.showWithGravity(
            'onAdvertisingStartFailed(' +
              endpointName +
              ',' +
              serviceId +
              ',' +
    ...
    this.subscribed_events.push(
      Nearby.onReceivePayload(
        ({serviceId, endpointId, payloadType, payloadId}) => {
          console.log(
            'onReceivePayload(' +
              serviceId +
              ',' +
            ...
            if (service.serviceId == serviceId) {
              const endpoints = service.endpoints.map(endpoint => {
                if (endpoint.id === endpointId) {
                  const payloads = endpoint
                    .get('payloads')
                    .set(
                      payloadId,
                      new Payload({payloadType, payloadId, playing: false}),
                    );
    ...
    this.subscribed_events.push(
      Nearby.onPayloadTransferUpdate(result => {
        const {serviceId, endpointId, payloadId} = result;
              ...
              if (endpoint.id === endpointId) {
                ...
                payloads.forEach((payload, k) => {
                  if (k === payloadId) {
                    ...
                    if (
                      payload.payloadType == Nearby_Payload.BYTES &&
                      payload.payloadStatus == PayloadTransferUpdate.SUCCESS
                    ) {
    ...
    // Connection
    this.subscribed_events.push(
      Nearby.onConnectionInitiatedToEndpoint(
        ({
          serviceId,
          endpointId,
          authenticationToken,
    ...
    this.subscribed_events.push(
      Nearby.onEndpointConnectionFailed(
        ({endpointId, serviceId, endpointName, statusCode}) => {
          ToastAndroid.showWithGravity(
            'onEndpointConnectionFailed(' +
              serviceId +
              ',' +
              endpointId +
              ',' +
    ...
    this.subscribed_events.push(
      Nearby.onDisconnectedFromEndpoint(
      ...
      if (service.serviceId == serviceId) {
        const endpoints = service.endpoints.map(endpoint => {
          if (endpoint.id === endpointId) {
            const payloads = endpoint
              .get('payloads')
              .set(payloadId, payload.set('playing', true));
            endpoint = endpoint.set('payloads', payloads);
          }
      ...
      if (service.serviceId == serviceId) {
        const endpoints = service.endpoints.map(endpoint => {
          if (endpoint.id === endpointId) {
            const payloads = endpoint
              .get('payloads')
              .set(payloadId, payload.set('playing', false));
            endpoint = endpoint.set('payloads', payloads);
          }
  ...
  handleSaveFile = (endpoint, payload) => {
    ...
    Nearby.saveFile(serviceId, endpointId, payloadId).then(data => {
  ...
  handleReadBytes = (endpoint, payload) => {
    ...
    Nearby.readBytes(serviceId, endpointId, payloadId).then(data => {
  ...
  handleSubmitCreateService = value => {
    ...
    this.setState({
      advertising: this.state.advertising.push(
        new Service({
          serviceId: serviceId,
  ...
  handleSubmitDiscoverService = value => {
    const serviceId = value;
    this.setState({
      discovering: this.state.discovering.push(
        new Service({
          serviceId: serviceId,
        }),
      ),
